...
contract L2Registry is ERC721, AccessControl {
    ...
    /// @param coinType The coin type to set address for
    /// @param value The address value
    function setAddr(bytes32 labelhash, uint256 coinType, bytes memory value)
        public
    ...
    /// @param key The record key
    /// @param value The record value
    function setText(bytes32 labelhash, string memory key, string memory value)
        public
        onlyTokenOperatorOrRegistrar(labelhash)
    {
    ...
    function setRecords(bytes32 labelhash, Text[] calldata texts, Addr[] calldata addrs, bytes calldata chash)
        external
        onlyTokenOperatorOrRegistrar(labelhash)
    {
        uint256 i;
