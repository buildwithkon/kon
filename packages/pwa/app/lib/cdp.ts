...
import { type Address, type BlockTag, type Hex, decodeAbiParameters, decodeFunctionData } from 'viem'
...
export async function willSponsor({
  ...
  if (entrypoint.toLowerCase() !== ENTRYPOINT_ADDRESS_V07.toLowerCase()) return false
  ...
  try {
    ...
    if (!code) {
      ...
      if (factoryAddress.toLowerCase() !== coinbaseSmartWalletFactoryAddress.toLowerCase()) return false
    } else {
