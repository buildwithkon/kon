...
export const getConfig = (ENV: Env) =>
  createConfig({
    ...
    transports: {
      ...
      [base.id]: http(`https://api.developer.coinbase.com/rpc/v1/base/${ENV.COINBASE_CLIENT_API_KEY}`),
      [baseSepolia.id]: http(
        `https://api.developer.coinbase.com/rpc/v1/base-sepolia/${ENV.COINBASE_CLIENT_API_KEY}`
      )
    },
